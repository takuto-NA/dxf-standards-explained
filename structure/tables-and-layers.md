...
- 色番号 `0` は「ByBlock」（ブロックの色を使用）を意味します。
- フラグ `70` のビット `1` が立っている場合、その画層は凍結（非表示）されます。

## ACI (AutoCAD Color Index)

DXFでは伝統的に `1` から `255` までのインデックスカラー（ACI）を使用します。
...
| `4` | 水色 (Cyan) | `#00FFFF` | |
| `5` | 青 (Blue) | `#0000FF` | |
| `6` | マゼンタ (Magenta) | `#FF00FF` | |
| `7` | 白/黒 (White/Black) | `#FFFFFF` または `#000000` | **背景色により反転します** |

### 実装上の注意：色番号7の「反転」
ACI 7番は、CADの背景色が黒の場合は「白」、背景色が白の場合は「黒」として表示される特殊な色です。パーサーやビューワーを実装する際は、描画キャンバスの背景色に合わせて色を出し分ける必要があります。

### True Color (RGB)
AutoCAD 2004 (AC1018) 以降では、グループコード `420` を使用して、24bitのTrue Colorを直接指定できます。
- **420**: `0xRRGGBB` 形式の整数値（例: `16711680` は赤）。
