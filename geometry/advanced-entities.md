...
**導入バージョン**: AutoCAD R14 (AC1014)

ハッチングは、閉じた領域をパターンや塗りつぶしで埋めるエンティティです。DXFの中で最も複雑な構造を持つエンティティの一つです。

### 境界パス（Boundary Paths）の構造

ハッチングの最も重要な点は、**「どの範囲を塗るか」**を定義する境界パスの多重構造です。

- **91**: 境界パスの数。
- **境界パスのループ**:
    - **92**: パスのフラグ（1=外部、0=デフォルト、16=最外周など）。
    - **93**: パスを構成するエッジ（線分、円弧など）の数。
    - **エッジデータ**: 直線、円弧、スプラインなどの幾何データが並びます。

### 島（Islands）の検出

ハッチングには「島」という概念があり、閉じたループの中にさらにループがある場合、そこを塗るか抜くかを指定できます。

- **75**: ハッチングスタイル（0=外側のみ、1=交互、2=全体を塗りつぶし）。

### 実装のアドバイス
HATCHは非常に複雑なため、パーサーを自作する場合はまず「SOLID（塗りつぶし）」のみをサポートし、複雑なパターン定義（コード `2` の名前参照）のレンダリングは後回しにするのが現実的です。
