...
バルジ $B$ は、2点間の直線（弦）からどれだけ円弧が「膨らんでいるか」を示します。
...
**数学的な関係:**
- **バルジの定義**: $B = \tan(\theta / 4)$ （$\theta$ は円弧の中心角）
- **膨らみの高さ $H$**: $H = (\text{弦の長さ} / 2) \times |B|$
- **半径 $R$**: $R = \frac{\text{弦の長さ}}{2} \times \frac{1 + B^2}{2|B|}$

実務的には、このバルジ値から「円の中心」と「半径」を逆算して、多くの描画ライブラリが期待する形式（中心、半径、開始角、終了角）に変換する必要があります。
